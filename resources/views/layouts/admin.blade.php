...
<head>
    ...
    <title>@yield('title','Admin - BilletPro')</title>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .sidebar-brand .logo-sub{font-size:9px;color:#a0aec0;text-transform:uppercase;letter-spacing:0.5px}
        ...
        .btn-icon{width:36px;height:36px;border-radius:8px;background:#f4f6f9;border:0.5px solid #e0e7ef;display:flex;align-items:center;justify-content:center;font-size:14px;cursor:pointer;text-decoration:none}

        /* STAT CARDS */
        .stat-card{background:#fff;border:0.5px solid #e0e7ef;border-radius:12px;padding:1.1rem}
        .stat-card-dark{background:#0f3460;border-radius:12px;padding:1.1rem;position:relative;overflow:hidden}
        .stat-label{font-size:11px;color:#a0aec0;margin-bottom:6px;font-weight:500}
        .stat-label-dark{font-size:10px;color:rgba(255,255,255,0.5);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:6px;font-weight:500}
        .stat-value{font-size:24px;font-weight:600;color:#0f1824}
        .stat-value-dark{font-size:20px;font-weight:700;color:#fff;line-height:1.2}
...
<div class="sidebar">
    <div class="sidebar-brand">
        ...
        <div>
            ...
            <div class="logo-sub">Édition Entreprise</div>
        </div>
    </div>
    ...
    <div class="sidebar-nav">
        ...
        <a href="{{ route('admin.clients') }}" class="nav-item-pro {{ request()->routeIs('admin.clients') ? 'active' : '' }}">
            <div class="nav-icon"><i class="fas fa-users" style="font-size:11px"></i></div>
            <span class="nav-label">Portail client</span>
        </a>
        <a href="{{ route('admin.organisateurs') }}" class="nav-item-pro {{ request()->routeIs('admin.organisateurs') ? 'active' : '' }}">
            <div class="nav-icon"><i class="fas fa-user-tie" style="font-size:11px"></i></div>
            <span class="nav-label">Outils organisateur</span>
        </a>
        <a href="{{ route('admin.evenements') }}" class="nav-item-pro {{ request()->routeIs('admin.evenements') ? 'active' : '' }}">
            <div class="nav-icon"><i class="fas fa-calendar-alt" style="font-size:11px"></i></div>
            <span class="nav-label">Console Admin</span>
        </a>
        <a href="{{ route('admin.statistiques') }}" class="nav-item-pro {{ request()->routeIs('admin.statistiques') ? 'active' : '' }}">
            <div class="nav-icon"><i class="fas fa-chart-bar" style="font-size:11px"></i></div>
            <span class="nav-label">Statistiques</span>
        </a>
        <a href="{{ route('admin.paiements') }}" class="nav-item-pro {{ request()->routeIs('admin.paiements') ? 'active' : '' }}">
            <div class="nav-icon"><i class="fas fa-file-alt" style="font-size:11px"></i></div>
            <span class="nav-label">Rapports</span>
        </a>
        <a href="{{ route('admin.categories') }}" class="nav-item-pro {{ request()->routeIs('admin.categories') ? 'active' : '' }}">
            <div class="nav-icon"><i class="fas fa-tags" style="font-size:11px"></i></div>
            <span class="nav-label">Catégories</span>
        </a>
        <a href="{{ route('admin.billets') }}" class="nav-item-pro {{ request()->routeIs('admin.billets') ? 'active' : '' }}">
            <div class="nav-icon"><i class="fas fa-ticket-alt" style="font-size:11px"></i></div>
            <span class="nav-label">Billets</span>
        </a>
        <a href="{{ route('admin.profil') }}" class="nav-item-pro {{ request()->routeIs('admin.profil') ? 'active' : '' }}">
            <div class="nav-icon"><i class="fas fa-user-cog" style="font-size:11px"></i></div>
            <span class="nav-label">Mon profil</span>
        </a>
        <a href="{{ route('admin.logout') }}" class="nav-item-pro danger">
    ...
    <div class="sidebar-footer">
        <div class="d-flex align-items-center gap-2">
            <div style="width:32px;height:32px;border-radius:50%;background:#0288d1;display:flex;align-items:center;justify-content:center;font-size:12px;color:#fff;font-weight:500">{{ strtoupper(substr(session('admin_nom','A'),0,2)) }}</div>
            <div>
                <div style="font-size:12px;font-weight:500;color:#0f1824">{{ session('admin_nom','Administrateur') }}</div>
                <div style="font-size:10px;color:#a0aec0">Super Admin</div>
            </div>
        </div>
    </div>
</div>
...
<div class="main-content">
    <div class="top-bar">
        ...
        <div class="d-flex align-items-center gap-2">
            <div style="background:#f4f6f9;border:0.5px solid #e0e7ef;border-radius:8px;padding:6px 14px;display:flex;align-items:center;gap:6px;font-size:12px;color:#718096">
                <i class="fas fa-search" style="font-size:11px"></i> Rechercher...
            </div>
            <div class="btn-secondary-pro ml-2">Assistance</div>
            <div class="btn-icon ml-2" style="position:relative">
                <i class="fas fa-bell" style="color:#a0aec0;font-size:14px"></i>
                <div style="position:absolute;top:6px;right:6px;width:7px;height:7px;background:#e53e3e;border-radius:50%;border:1.5px solid #fff"></div>
            </div>
            <div class="btn-icon ml-1">
                <i class="fas fa-cog" style="color:#a0aec0;font-size:14px"></i>
            </div>
            <div style="width:32px;height:32px;border-radius:50%;background:#0288d1;display:flex;align-items:center;justify-content:center;font-size:12px;color:#fff;font-weight:500;margin-left:4px">{{ strtoupper(substr(session('admin_nom','A'),0,2)) }}</div>
        </div>
    </div>
